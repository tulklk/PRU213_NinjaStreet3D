...
public class UIManager : MonoBehaviour
{
    ...
    // =========================
    [Header("Nitro (Slider)")]
    [Tooltip("Kéo Slider Nitro trong UI vào đây")]
    public Slider nitroSlider;                    // Min=0, Max=1
    [Range(1f, 1.5f)] public float nitroPulseScale = 1.1f;
    public float nitroPulseDuration = 0.15f;
    private Tween nitroPulseTween;
    ...
    // ======= PUBLIC API: gọi từ GameManager/Player =======
    /// <summary>
    /// Set giá trị thanh Nitro theo tỉ lệ 0..1
    /// </summary>
    public void SetNitroFill01(float value01)
    {
        if (nitroSlider == null) return;

        value01 = Mathf.Clamp01(value01);
        nitroSlider.value = value01;

        // Nháy nhẹ khi vừa đầy bình
        if (Mathf.Approximately(value01, 1f))
            PulseNitro();
    }

    /// <summary>
    /// Reset thanh Nitro về 0
    /// </summary>
    public void ResetNitro()
    {
        if (nitroSlider != null) nitroSlider.value = 0f;
    }

    private void PulseNitro()
    {
        if (nitroSlider == null) return;
        var rt = nitroSlider.GetComponent<RectTransform>();
        if (rt == null) return;

        nitroPulseTween?.Kill();
        rt.localScale = Vector3.one;
        // Nháy 1 cái cho vui mắt
        nitroPulseTween = rt.DOPunchScale(Vector3.one * (nitroPulseScale - 1f), nitroPulseDuration, 1, 0f);
    }
    // ======= END NITRO =======
    ...
    public void ReturnHome() { SceneManager.LoadScene("Play"); }

    public void Quit() { GameManager.instance.RetryGame(); }
    ...
    public void SettingButton() { UIAnimator.Show(settingUI); }

    public void ShowDenyBuyUI() { UIAnimator.Show(denyUpdateUI); }
    public void CloseDenyBuyUI() { UIAnimator.Hide(denyUpdateUI); }

    public void ShowDenyUpdateUI() { UIAnimator.Show(denyUpdateUI); }
    public void CloseDenyUpdateUI() { UIAnimator.Hide(denyUpdateUI); }

    public void CloseSetting() { UIAnimator.Hide(settingUI); }

    //Boost UI
    public void TurnBoostUi() { boostUI.SetActive(true); }
    public void StopBoostUI() { boostUI.SetActive(false); }
