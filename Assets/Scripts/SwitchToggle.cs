...
//    [Header("Layout")]
//    [Tooltip("Canh sát 2 đầu bo tròn (khuyên dùng)")]
//    [SerializeField] private bool alignToRoundedEnds = true;
//    [Tooltip("Chừa mép rất nhỏ nếu muốn (px)")]
//    [SerializeField] private float edgeInset = 0f;
//    [Tooltip("Nếu KHÔNG canh theo bo tròn, dùng padding 2 bên")]
//    [SerializeField] private float padding = 6f;

//    [Header("Text Colors")]
//    public Color onTextColor = new Color(37f / 255f, 37f / 255f, 37f / 255f); // #252525
...
//    private float target;          // 0..1 (mục tiêu)
//    private float leftX, rightX;   // biên di chuyển hợp lệ của handle
...
//        {
//            Debug.LogWarning("[SwitchToggle] Chưa gán Fill/Handle.");
//            enabled = false;
//            return;
//        }
...
//        handleRect.pivot = new Vector2(0.5f, 0.5f); // pivot giữa để tính bán kính dễ
//    }
...
//        RecalculateTravel();
//        target = isOn ? 1f : 0f;
//        ApplyInstant();      // đặt position + fillAmount
//        UpdateLabel();       // cập nhật chữ
//        ApplyFillColor(true); // đặt màu fill ngay lập tức theo trạng thái ban đầu
//    }
...
//        {
//            // SÁT VIỀN: trái = bán kính + inset, phải = bgWidth - bán kính - inset
//            float radius = knobW * 0.5f;
//            leftX = radius + edgeInset;
//            rightX = bgWidth - radius - edgeInset;
//        }
//        else
//        {
//            // Dùng padding hai bên
...
//        // Lerp fill amount
//        fillImage.fillAmount = Mathf.Lerp(fillImage.fillAmount, target, Time.deltaTime * fillLerpSpeed);

//        // Lerp handle position
//        float targetX = Mathf.Lerp(leftX, rightX, target);
//        float newX = Mathf.Lerp(handleRect.anchoredPosition.x, targetX, Time.deltaTime * handleLerpSpeed);
//        handleRect.anchoredPosition = new Vector2(newX, 0f); // y=0 để luôn giữa

//        // Lerp fill color (đỏ <-> xanh)
...
//        float x = Mathf.Lerp(leftX, rightX, target);
...
//    {
//        if (fillImage == null) return;
//        Color targetCol = isOn ? fillOnColor : fillOffColor;

//        if (instant)
//            fillImage.color = targetCol;
//        else
//            fillImage.color = Color.Lerp(fillImage.color, targetCol, Time.deltaTime * colorLerpSpeed);
//    }
//}
